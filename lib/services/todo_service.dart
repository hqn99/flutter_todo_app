...
class TodoService {
  ...
  final SharedPreferences _pref; // データ保存の仕組み

  TodoService(this._pref);

  // 保存されているTODOリストを読み込む（非同期処理）
  Future<List<Todo>> getTodos() async {
    // 保存されているJSONデータを取得
    final String? todosJson = _pref.getString(_storageKey);
  ...
  Future<void> saveTodos(List<Todo> todos) async {
    ...
    // JSON文字列に変換して保存
    await _pref.setString(_storageKey, jsonEncode(encoded));
  }
}
